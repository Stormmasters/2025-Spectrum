...
public class FakeArm extends Mechanism {

    public static class FakeArmConfig extends Config {
        ...
        public FakeArmConfig modifyMotorConfig(TalonFX motor) {
    ...
    private FakeArmConfig config;
    private CANcoder m_CANcoder;
    @Getter private FakeArmSim sim;
    CANcoderSimState canCoderSim;

    public FakeArm(FakeArmConfig config) {
        super(config);
        this.config = config;
    ...
    public void setupStates() {
        FakeArmStates.setStates();
    }

    public void setupDefaultCommand() {
        FakeArmStates.setupDefaultCommand();
    }
    ...
    public Command zeroFakeArmRoutine() {
        return new FunctionalCommand(
                        ...
                        () -> false, // isFinished
                        this) // requirement
                .withName("FakeArm.zeroPivotRoutine");
    }

    /** Holds the position of the pivot. */
    public Command runHoldFakeArm() {
        return new Command() {
            double holdPosition = 0; // rotations

            // constructor
            {
                setName("FakeArm.holdPosition");
                addRequirements(FakeArm.this);
            }
    ...
    private void simulationInit() {
        if (isAttached()) {
            sim = new FakeArmSim(motor.getSimState(), RobotSim.leftView);
    ...
    class FakeArmSim extends ArmSim {
        public FakeArmSim(TalonFXSimState fakeArmMotorSim, Mechanism2d mech) {
            super(
                    new ArmConfig(
                                    config.pivotX,
                                    config.pivotY,
